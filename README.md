...
- 🔄 切换 Steam 账号：断网 / 删除旧账号订阅文件 / 重新联网，避免旧订阅被自动恢复
...
侧边栏「切换账号」（位于「统计看板」下方）用于切换 Steam 账号并清理旧账号的残留订阅：

1. 在「WallPaper Engine 安装路径」填写 `...\steamapps\common\wallpaper_engine\bin`（也可从设置继承），用于识别 Wallpaper Engine 的联网状态。
2. 用开关控制 Wallpaper Engine **联网 / 断网**（断网 = 创建 `nosteam.txt`；联网 = 删除它）。
3. 在「选择账号」下拉里选择目标 Steam 用户，查看其订阅文件（`431960_subscriptions.vdf`）状态。
4. 点击 **删除订阅文件** 移除该账号的订阅记录（`userdata\<id>\ugc\431960_subscriptions.vdf`）。

> 推荐流程：**先断网 → 删除旧账号的订阅文件 → 重新联网**。这样可避免旧账号的订阅在重新联网后被自动下载或恢复。删除前会要求确认。
